...
(ref:bessel-function) 贝塞尔函数 $\mathcal{K}_{\kappa}(u)$ 在区间 $(10^{-8}, 10^2)$ 和 $(0, 4)$ 上的图像
(ref:bessel-left) 区间 $(10^{-8}, 10^2)$ 上的贝塞尔函数 $\mathcal{K}_{\kappa}(u)$
(ref:bessel-right) 区间 $(0, 4)$ 上的贝塞尔函数 $\mathcal{K}_{\kappa}(u)$

```{r bessel-function, dev='tikz', fig.cap="(ref:bessel-function)", fig.subcap=c("(ref:bessel-left) ", "(ref:bessel-right)"), echo=FALSE, fig.show='hold', out.width="50%", fig.width=3.5, fig.height=3.5, cache=TRUE, fig.process=to_png}
x0 <- 2^(-20:10)
nus <- c(2:5, 10, 20)
...
head(iris, 10) |>
  ...
  kable(caption = "iris 数据集（部分）", booktabs = TRUE,
...
```{r package-info, tidy=TRUE, tidy.opts=list(width.cutoff=50)}
